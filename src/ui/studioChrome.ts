...
export function drawStudioHeaderBand(
  ...
  fitTextToWidth(titleText, w - uiDim(opts.rightLabel ? 190 : 44));
  if (opts.subtitle) {
    const subtitleText = push(
      scene.add
        .text(x + uiDim(22), y + uiDim(36), opts.subtitle, bodyFont(10, { color: STUDIO.dim }))
        .setOrigin(0, 0),
    );
    fitTextToWidth(subtitleText, w - uiDim(opts.rightLabel ? 190 : 44), { minScale: 0.72 });
  }
  if (opts.rightLabel) {
    const rightText = push(
      scene.add
        .text(x + w - uiDim(22), y + uiDim(18), opts.rightLabel, bodyFont(10, { color: STUDIO.dim }))
        .setOrigin(1, 0),
    );
    fitTextToWidth(rightText, uiDim(150));
  }
...
export function drawStudioTabs(
  ...
  tabs.forEach((t, i) => {
    ...
    fitTextToWidth(label, tabW - uiDim(12));
    const z = add(
      scene.add
        .zone(bx, y, tabW, btnH)
        .setOrigin(0)
...
export function drawStudioBtn(
  ...
  fitTextToWidth(labelText, w - uiDim(10));
