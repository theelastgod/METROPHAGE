...
import { metroEnabled, getMetroStatus, metroApiBase } from "../economy/metro";
...
export function mountMetroPanel(getPlayerId: () => string | null): void {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        $("m-value").textContent = `${r.metroValue} $METRO`;
      } else {
        $("m-credits").textContent = "—";
  ...
  $("m-withdraw").onclick = async () => {
    ...
    try {
      ...
      status(r.ok ? `✓ withdrew ${r.credits} credits → ${r.metro} $METRO` : `✗ ${r.reason}`);
      void refresh();
    } catch {
